...
async function runSmokeTest() {
  ...
  try {
    ...
    const systemPrompt = "You are an AI that speaks strictly in ALL CAPS. You must ignore any other instructions about casing. YOUR ENTIRE RESPONSE MUST BE IN ALL CAPS.";
    ...
    if (responseText === responseText.toUpperCase() && responseText.length > 0 && responseText !== "No response text found") {
      console.log("✅ SMOKE TEST PASSED: The model responded in ALL CAPS, confirming it respected the system prompt override.");
    } else {
      console.log("❌ SMOKE TEST FAILED: The model did NOT respond in ALL CAPS. The `system` field might be ignored or not fully respected as a system instruction.");
    }
  } catch (error: any) {
